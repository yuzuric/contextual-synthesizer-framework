# Contextual Synthesizer Framework
...
`contextual-synthesizer-framework` is built on top of [Xiaomi MiMo](https://platform.xiaomimimo.com), the open-source large language model series from Xiaomi. The project demonstrates how to integrate MiMo into production systems using its OpenAI-compatible API.
...
```python
import asyncio
from contextual_synthesizer_framework.client import MiMoClient
...
├── config.yaml
├── main.py
├── src/contextual_synthesizer_framework/
│   ├── retriever.py
│   ├── client.py
│   └── config.py
├── tests/
